/**
 * Gemini-powered nutrition service (Google LLM)
...
export class GeminiNutritionService {
  async reverseEngineerItem(
    ...
    console.log(`🔮 Calling Gemini API: ${restaurantName} — ${itemName} (${addOns.length} add-ons)`);
    ...
    const responseSchema = {
      ...
      required: ["baseItem", "addOns"],
    };

    const systemInstruction = `You are an expert Egyptian sports nutritionist specializing in Egyptian and international cuisine.
...
Customizations to calculate: ${JSON.stringify(addOns)}`;
    ...
    if (!parsed.baseItem || !parsed.baseItem.calories || typeof parsed.baseItem.calories.min !== "number") {
      throw new Error("Gemini returned invalid JSON response structure");
    }
  ...
  private getBaseItemMacros(itemName: string): BaseMacroRange {
    ...
    if (
      ...
      baseMacros = {
        caloriesMin: 250, caloriesMax: 320,
      ...
      baseMacros = {
        caloriesMin: 400, caloriesMax: 550,
        proteinMin: 20,   proteinMax: 30,
        carbsMin: 45,     carbsMax: 60,
        fatsMin: 15,      fatsMax: 25,
      };
    }
